...
±±³Marcelo     ³26/05/2009³012430³ Tratamento para textos acentuados quando ³±±
±±³            ³          ³  2009³ a impressao for Direta na Porta - LPT1.  ³±±
±±³Allyson M.  ³05/06/2009³013711³ Inclusao de informacoes na exibicao do   ³±±
±±³            ³          ³  2009³ relatorio de impressao zebrado (CNPJ  e  ³±±
±±³            ³          ³      ³ endereco da empresa.				    	³±±
±±ÀÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÚÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿±±
±±³Marcelo     ³02/07/2009³016296/2009³Ajuste em fLancaZ() e fRodape() para nao  ³±±
±±³            ³          ³           ³chamar a funcao fTAcento quando nao tiver ³±±
...
USER Function XGPER030(lTerminal,cFilTerminal,cMatTerminal,cMesAnoRef,nRecTipo,cSemanaTerminal)
...
//oPrint:=TMSPrinter():New("Recibo de pagamento de salario")
//oPrint:SetPortrait()        
//oPrint:SetSize(210,280)    
...
oFont09b  := TFont():New("Arial",9,9,.T.,.T.,5,.T.,5,.T.,.F.)
oFont09   := TFont():New("Arial",9,9,.T.,.F.,5,.T.,5,.T.,.F.)
oFont08b  := TFont():New("Arial",8,8,.T.,.T.,5,.T.,5,.T.,.F.)
oFont08   := TFont():New("Arial",8,8,.T.,.F.,5,.T.,5,.T.,.F.)
oFont07b  := TFont():New("Arial",7,7,.T.,.T.,5,.T.,5,.T.,.F.)
oFont07   := TFont():New("Arial",7,7,.T.,.F.,5,.T.,5,.T.,.F.)
...
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
If !U_OpenSrc( cMesArqRef, @cAliasMov, @aOrdBag, @cArqMov, @dDataRef , NIL ,lTerminal )
...
While SRA->( !Eof() .And. &cInicio <= cFim )
	...
	ElseIf nTipRel == 4 .and. !( lTerminal )
		fImpresGr()
	Endif
...
±±³Fun‡…o    ³fImpressao³ Autor ³ R.H. - Ze Maria       ³ Data ³ 14.03.95 ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
±±³Descri‡…o ³ IMRESSAO DO RECIBO EM FOLHA SULFITE                       ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
±±³Sintaxe   ³ fImpresGr()                                               ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
±±³Parametros³                                                            ³±±
...
Local lPulouPag:= .F.
Local nParteUtil:= 0
Private nLinhas:=14              // Numero de Linhas do Miolo do Recibo
...
FOR i:= 1 TO 2
   ...
   LI:= 0
   IMPLYOUTFO(nParteUtil)
   fCabecGr(nParteUtil)
   nContr:= 0
   nContrT:= 0
	For nConta = 1 To Len(aLanca)
	   fLancaGr(nConta, nParteUtil)
		nContr ++
		nContrT ++
		If nContr = nLinhas .And. nContrT < Len(aLanca)
			nContr:=0
			Ordem_Rel ++
			fContinGr(lLinhaRoda, nParteUtil)
//		     fRodapeGr(lLinhaRoda, nParteUtil )
			   LI:= 0 
			  	IF nParteUtil == 2
					oPrint:EndPage()           
//			lPulouPag:= .F.
			   ENDIF
			   nParteUtil:= If( nParteUtil == 1, 2, 1 )
			   IMPLYOUTFO(nParteUtil)
           fCabecGr(nParteUtil)
           lPulouPag:= If( nParteUtil == 2, .T., .F. )
		Endif
	Next nConta
//	  	IF lPulouPag == 2
//			oPrint:EndPage()           
...
//	Li+=IF( lLinhaRoda := (nLinhas-nContr) == 0,1,(nLinhas-nContr))
   fRodapeGr(lLinhaRoda, nParteUtil )
   IF i = 2 .OR. (i=1 .AND. nParteUtil = 2)
		oPrint:EndPage()   
   ENDIF
   nParteUtil:= If( nParteUtil == 1, 2, 1 )
NEXT                           
...
STATIC FUNCTION IMPLYOUTFO(nVia)
Local cLogo          := ""
...
If !File( cLogo )
	cLogo := cStartPath + "LGRL" + SM0->M0_CODIGO + ".BMP" 						// Empresa
     ...
     ELSE
		...
		nCol        := 1
     ENDIF   
     oPrint:Box(SetLin(nLin),SetCol(nCol:=04),SetLin(nLin+05),SetCol(nCol:=157 ))       //Da linha 06 a 10
	 oPrint:SayBitmap(SetLin(nLin+1),SetCol(nCol:=5),cLogo,250,130) 
     oPrint:Say(SetLin(nLin+01),SetCol(nCol:=116),"RECIBO DE PAGAMENTO DE SALÁRIO",oFont09b)		           //Linha 07 
//     oPrint:Say(SetLin(nLin+01),SetCol(nCol:=25),"SOLUTION INTEGRAÇÕES DE TI ",oFont09b)		           //Linha 07 
//     oPrint:Say(SetLin(nLin+02),SetCol(nCol:=25),"SOLUTION INTEGRAÇÕES DE TI ",oFont09b)		           //Linha 07 
//     oPrint:Say(SetLin(nLin+03),SetCol(nCol:=25),"SOLUTION INTEGRAÇÕES DE TI ",oFont09b)		           //Linha 07 
     oPrint:Box(SetLin(nLin+05),SetCol(nCol:=04),SetLin(nLin+06),SetCol(nCol:=157) )    //Da linha 10 a 12
     oPrint:Say(SetLin(nLin+05),SetCol(nCol:=06),"Código",oFont07b)		           //Linha 07 
     oPrint:Say(SetLin(nLin+05),SetCol(nCol:=18),"Nome do Funcionário",oFont07b)		           //Linha 07 
     oPrint:Say(SetLin(nLin+05),SetCol(nCol:=70),"CBO",oFont07b)		           //Linha 07 
     oPrint:Say(SetLin(nLin+05),SetCol(nCol:=80),"Emp",oFont07b)		           //Linha 07 
     oPrint:Say(SetLin(nLin+05),SetCol(nCol:=98),"Local",oFont07b)		           //Linha 07 
     oPrint:Say(SetLin(nLin+05),SetCol(nCol:=116),"Depto",oFont07b)		           //Linha 07 
     oPrint:Say(SetLin(nLin+05),SetCol(nCol:=134),"Sessão",oFont07b)		           //Linha 07 
     oPrint:Say(SetLin(nLin+05),SetCol(nCol:=152),"FL",oFont07b)		           //Linha 07 
     oPrint:Box(SetLin(nLin+06),SetCol(nCol:=04),SetLin(nLin+10),SetCol(nCol:=157) )    //Da linha 12 a 17      
//     oPrint:Say(SetLin(nLin+07),SetCol(nCol:=7),"EDSON SHIDEKI KOKADO ",oFont09b)		           //Linha 07 
//     oPrint:Say(SetLin(nLin+08),SetCol(nCol:=7),"FUNÇÃO: ANALISTA DE SISTEMA ",oFont09b)		           //Linha 07 
     oPrint:Box(SetLin(nLin+10),SetCol(nCol:=04),SetLin(nLin+11),SetCol(nCol:=20) )    //Da linha 10 a 12
     oPrint:Say(SetLin(nLin+10),SetCol(nCol:=10),"Código",oFont07b)		           //Linha 07 
     oPrint:Box(SetLin(nLin+10),SetCol(nCol:=20),SetLin(nLin+11),SetCol(nCol:=109) )    //Da linha 10 a 12
     oPrint:Say(SetLin(nLin+10),SetCol(nCol:=50),"Descrição",oFont07b)		           //Linha 07 
     oPrint:Box(SetLin(nLin+10),SetCol(nCol:=109),SetLin(nLin+11),SetCol(nCol:=120) )    //Da linha 10 a 12
     oPrint:Say(SetLin(nLin+10),SetCol(nCol:=111),"Referência",oFont07b)		           //Linha 07 
     oPrint:Box(SetLin(nLin+10),SetCol(nCol:=120),SetLin(nLin+11),SetCol(nCol:=141) )    //Da linha 10 a 12
     oPrint:Say(SetLin(nLin+10),SetCol(nCol:=128),"Vencimentos",oFont07b)		           //Linha 07 
     oPrint:Box(SetLin(nLin+10),SetCol(nCol:=141),SetLin(nLin+11),SetCol(nCol:=157) )    //Da linha 10 a 12
     oPrint:Say(SetLin(nLin+10),SetCol(nCol:=147),"Descontos",oFont07b)		           //Linha 07 
     oPrint:Box(SetLin(nLin+11),SetCol(nCol:=04),SetLin(nLin+26),SetCol(nCol:=20) )    //Da linha 10 a 12
     oPrint:Box(SetLin(nLin+11),SetCol(nCol:=20),SetLin(nLin+26),SetCol(nCol:=109) )    //Da linha 10 a 12
     oPrint:Box(SetLin(nLin+11),SetCol(nCol:=109),SetLin(nLin+26),SetCol(nCol:=120) )    //Da linha 10 a 12
     oPrint:Box(SetLin(nLin+11),SetCol(nCol:=120),SetLin(nLin+26),SetCol(nCol:=141) )    //Da linha 10 a 12
     oPrint:Box(SetLin(nLin+11),SetCol(nCol:=141),SetLin(nLin+26),SetCol(nCol:=157) )    //Da linha 10 a 12
     oPrint:Box(SetLin(nLin+26),SetCol(nCol:=04),SetLin(nLin+30),SetCol(nCol:=120) )    //Da linha 10 a 12
     oPrint:Box(SetLin(nLin+26),SetCol(nCol:=120),SetLin(nLin+28),SetCol(nCol:=141) )    //Da linha 10 a 12
     oPrint:Say(SetLin(nLin+26),SetCol(nCol:=123),"Total de Vencimentos",oFont07b)		           //Linha 07 
     oPrint:Box(SetLin(nLin+26),SetCol(nCol:=141),SetLin(nLin+28),SetCol(nCol:=157) )    //Da linha 10 a 12
     oPrint:Say(SetLin(nLin+26),SetCol(nCol:=142),"Total de Descontos",oFont07b)		           //Linha 07
      
//     oPrint:Box(SetLin(nLin+28),SetCol(nCol:=04),SetLin(nLin+30),SetCol(nCol:=124) )    //Da linha 10 a 12
     oPrint:Box(SetLin(nLin+28),SetCol(nCol:=120),SetLin(nLin+30),SetCol(nCol:=157) )    //Da linha 10 a 12
//     oPrint:Box(SetLin(nLin+28),SetCol(nCol:=147),SetLin(nLin+30),SetCol(nCol:=170) )    //Da linha 10 a 12
     oPrint:Say(SetLin(nLin+28.5),SetCol(nCol:=122),"Valor Liquido ===>",oFont09b)		           //Linha 07 
     oPrint:Box(SetLin(nLin+30),SetCol(nCol:=04),SetLin(nLin+31),SetCol(nCol:=157) )    //Da linha 10 a 12
     oPrint:Say(SetLin(nLin+30),SetCol(nCol:=10),"Salário Base",oFont07b)		           //Linha 07
     oPrint:Say(SetLin(nLin+30),SetCol(nCol:=38),"Sal. Contr. I.N.S.S.",oFont07b)		           //Linha 07
     oPrint:Say(SetLin(nLin+30),SetCol(nCol:=66),"Base de F.G.T.S.",oFont07b)		           //Linha 07
     oPrint:Say(SetLin(nLin+30),SetCol(nCol:=94),"F.G.T.S. do mês",oFont07b)		           //Linha 07
     oPrint:Say(SetLin(nLin+30),SetCol(nCol:=122),"Base Calc. I.R.F.",oFont07b)		           //Linha 07
     oPrint:Say(SetLin(nLin+30),SetCol(nCol:=143),"Faixa I.R.F.",oFont07b)		           //Linha 07
     oPrint:Box(SetLin(nLin+31),SetCol(nCol:=04),SetLin(nLin+33),SetCol(nCol:=157) )    //Da linha 10 a 12
     oPrint:Box(SetLin(nLin+33),SetCol(nCol:=04),SetLin(nLin+40),SetCol(nCol:=157) )    //Da linha 10 a 12
     oPrint:Say(SetLin(nLin+34),SetCol(nCol:=07),"Declaro ter recebido a importância liquida discriminada neste recibo.",oFont09b)		           //Linha 07
     oPrint:Say(SetLin(nLin+37),SetCol(nCol:=07),"_______/_______/_______",oFont09b)		           //Linha 07
     oPrint:Say(SetLin(nLin+38),SetCol(nCol:=10),"         DATA",oFont09b)		           //Linha 07
     oPrint:Say(SetLin(nLin+37),SetCol(nCol:=40),"______________________________________________________________",oFont09b)		           //Linha 07
     oPrint:Say(SetLin(nLin+38),SetCol(nCol:=45),"                  ASSINATURA DO FUNCIONÁRIO",oFont09b)		           //Linha 07
//     oPrint:Say(SetLin(nLin+01),SetCol(nCol:=085),SC5->C5_NUM,oFont14b)                 //Linha 07
//     oPrint:Say(SetLin(nLin+01),SetCol(nCol:=095),"  - PSP: " + SC5->C5_PROJTV1,oFont14b)		              //Linha 07           
//     oPrint:Box(SetLin(nLin+04),SetCol(nCol:=04),SetLin(nLin+06),SetCol(nCol:=155) )    //Da linha 10 a 12
//     oPrint:Box(SetLin(nLin+06),SetCol(nCol:=04),SetLin(nLin+12),SetCol(nCol:=80) )     //Da linha 12 a 17
//     oPrint:Say(SetLin(nLin+07),SetCol(nCol:=05),"1. Date of Exportation ",oFont09b)	  //Linha 13
//     oPrint:Say(SetLin(nLin+07),SetCol(nCol:=81),"2. Export References ",oFont09b)		  //Linha 13      
//     oPrint:Box(SetLin(nLin+15),SetCol(nCol:=80),SetLin(nLin+18),SetCol(nCol:=155) )    //Da linha 12 a 17 
//     oPrint:Box(SetLin(nLin + 18),SetCol(nCol:=80),SetLin(nLin+36),SetCol(nCol:=155) )  //Da linha 13 a 20      
RETURN NIL



//Função para poder realizar a quebrade linha
Static Function SetLin(nLin)
Local nResult:=0
//nresult     :=     nLin * 43
nresult     :=     nLin * 33
Return (nResult) 

//Função para poder controlar as colunas
Static Function SetCol(nCol)
Local nResCol     :=0
//nResCol     :=     nCol * 21
nResCol     :=     nCol * 15
Return (nResCol)  
...
@ LI,08 PSAY If( lLpt1, fTAcento(Left(SRA->RA_NOME,28)), Left(SRA->RA_NOME,28) )
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿±±
±±³Fun‡…o    ³fCabecGr    ³ Autor ³ R.H. - Edson Shideki  ³ Data ³ 14.03.95 ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß*/
Static Function fCabecGr(nVia)   		// Cabecalho do Recibo
Local cCodFunc		:= ""		//-- codigo da Funcao do funcionario
Local cDescFunc		:= ""		//-- Descricao da Funcao do Funcionario
...
//LI ++
//oPrint:Say(SetLin(nLin+01),SetCol(nCol:=25),&cNormal+DESC_Fil,oFont09b)		           //Linha 07 
oPrint:Say(SetLin(nLin+01),SetCol(nCol:=25),DESC_Fil,oFont09b)		           //Linha 07 
//@ LI,01 PSAY &cNormal+DESC_Fil
//LI ++
oPrint:Say(SetLin(nLin+02),SetCol(nCol:=25),DESC_END,oFont09b)		           //Linha 07 
//@ LI,01 PSAY DESC_END
//LI ++
oPrint:Say(SetLin(nLin+03),SetCol(nCol:=25),DESC_CGC,oFont09b)		           //Linha 07 
//@ LI,01 PSAY DESC_CGC

If !Empty(Semana) .And. Semana # '99' .And.  Upper(SRA->RA_TIPOPGT) == 'S'
   oPrint:Say(SetLin(nLin+03),SetCol(nCol:=75),STR0013 + Semana + ' (' + cSem_De + STR0014 + cSem_Ate + ')',oFont09b)		           //Linha 07 
//	@ Li,37 pSay STR0013 + Semana + ' (' + cSem_De + STR0014 + ;	//'Semana '###' a '
//	cSem_Ate + ')'
Else
   oPrint:Say(SetLin(nLin+03),SetCol(nCol:=135),UPPER(MesExtenso(MONTH(dDataRef)))+"/"+STR(YEAR(dDataRef),4),oFont09b)		           //Linha 07 
//	@ LI,55 PSAY MesExtenso(MONTH(dDataRef))+"/"+STR(YEAR(dDataRef),4)
EndIf

//LI +=2
oPrint:Say(SetLin(nLin+07),SetCol(nCol:=7),ALLTRIM(SRA->RA_Mat) ,oFont09)		           //Linha 07 
oPrint:Say(SetLin(nLin+07),SetCol(nCol:=18),If( lLpt1, fTAcento(Left(SRA->RA_NOME,28)), Left(SRA->RA_NOME,28) ),oFont09)		           //Linha 07 
//@ LI,01 PSAY SRA->RA_Mat
//oPrint:Say(SetLin(nLin+07),SetCol(nCol:=24),If( lLpt1, fTAcento(Left(SRA->RA_NOME,28)), Left(SRA->RA_NOME,28) ),oFont09)		           //Linha 07 
//@ LI,08 PSAY If( lLpt1, fTAcento(Left(SRA->RA_NOME,28)), Left(SRA->RA_NOME,28) )
oPrint:Say(SetLin(nLin+07),SetCol(nCol:=68),fCodCBO(SRA->RA_FILIAL,cCodFunc ,dDataRef),oFont09)		           //Linha 07 
//@ LI,37 PSAY fCodCBO(SRA->RA_FILIAL,cCodFunc ,dDataRef)
oPrint:Say(SetLin(nLin+07),SetCol(nCol:=80),SRA->RA_Filial,oFont09)		           //Linha 07 
//@ LI,44 PSAY SRA->RA_Filial
oPrint:Say(SetLin(nLin+07),SetCol(nCol:=93),PADC(ALLTRIM(SRA->RA_CC),20),oFont09)		           //Linha 07 
//@ LI,47 PSAY PADC(ALLTRIM(SRA->RA_CC),20)
oPrint:Say(SetLin(nLin+07),SetCol(nCol:=115),STRZERO(ORDEM_REL,2),oFont09)		           //Linha 07 
//@ LI,67 PSAY ORDEM_REL PICTURE "99"
//LI ++

cDet := 'Função:        ' + ALLTRIM(cCodFunc)+' - '						//-- Funcao
cDet += If( lLpt1, fTAcento(cDescFunc), cDescFunc ) + '       '
cDet += DescCc(SRA->RA_CC,SRA->RA_FILIAL) + '        '
cDet += STR0016 + SRA->RA_CHAPA					//'CHAPA: '
oPrint:Say(SetLin(nLin+08),SetCol(nCol:=7),If( lLpt1, fTAcento(cDet), cDet ),oFont09)		           //Linha 07 
//@ Li,01 pSay If( lLpt1, fTAcento(cDet), cDet )
...
LI ++
//@ LI,00  PSAY STR0123 + Desc_CGC	//"| CNPJ      : "
@ LI,131 PSAY "|"

LI ++
//@ LI,00  PSAY STR0124 + Desc_EndC	//"| Endereco  : "
@ LI,131 PSAY "|"   
...
@ LI,30  PSAY STR0024 + If( lLpt1, fTAcento(SRA->RA_NOME), SRA->RA_NOME )	//"Nome  : "
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß*/
Static Function fLancaGr(nConta, nVia)   // Impressao dos Lancamentos

Local cString := Transform(aLanca[nConta,5],'@R 9,999,999,999.99')
Local nCol := If(aLanca[nConta,1]="P",43,If(aLanca[nConta,1]="D",57,27))
...
Li ++
oPrint:Say(SetLin(nLin+10+LI),SetCol(nCol:=10),aLanca[nConta,2],oFont09)		           //Linha 07 
//@ LI,01 PSAY aLanca[nConta,2]
oPrint:Say(SetLin(nLin+10+LI),SetCol(nCol:=24),aLanca[nConta,3],oFont09)		           //Linha 07 
//@ LI,05 PSAY aLanca[nConta,3]
If aLanca[nConta,1] # "B"        // So Imprime se nao for base
   oPrint:Say(SetLin(nLin+10+LI),SetCol(nCol:=117),TRANSFORM(aLanca[nConta,4],"999.99"),oFont09,,,,PAD_RIGHT)		           //Linha 07 
//	@ LI,36 PSAY TRANSFORM(aLanca[nConta,4],"999.99")
Endif
//nCol := If(aLanca[nConta,1]="P",127,If(aLanca[nConta,1]="D",144,27))
nCol := If(aLanca[nConta,1]="P",138,If(aLanca[nConta,1]="D",155,27))
oPrint:Say(SetLin(nLin+10+LI),SetCol(nCol),cString,oFont09,,,,PAD_RIGHT)		           //Linha 07 
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß*/
Static Function fRodapeGr(lLinhaRoda, nVia)    // Rodape do Recibo
...
Endif
//
// multitek motivo centralizar data de aniversario.
//

IF EMPTY(DESC_MSG1) .AND. EMPTY(DESC_MSG2) .AND. EMPTY(DESC_MSG3)
oPrint:Say(SetLin(nLin+26),SetCol(nCol:=7),If( lLpt1 .And. !Empty(DESC_MSG1), fTAcento(DESC_MSG1), DESC_MSG1 ),oFont09)		           //Linha 07 
//@ LI,05 PSAY If( lLpt1 .And. !Empty(DESC_MSG1), fTAcento(DESC_MSG1), DESC_MSG1 )
LI ++
oPrint:Say(SetLin(nLin+27),SetCol(nCol:=7),If( lLpt1 .And. !Empty(DESC_MSG2), fTAcento(DESC_MSG2), DESC_MSG2 ),oFont09)		           //Linha 07 
//@ LI,05 PSAY If( lLpt1 .And. !Empty(DESC_MSG2), fTAcento(DESC_MSG2), DESC_MSG2 )
oPrint:Say(SetLin(nLin+27),SetCol(nCol:=127),TRANSFORM(TOTVENC,'@R 9,999,999,999.99'),oFont09)		           //Linha 07 
//@ LI,42 PSAY TOTVENC PICTURE cPict1
oPrint:Say(SetLin(nLin+27),SetCol(nCol:=144),TRANSFORM(TOTDESC,'@R 9,999,999,999.99'),oFont09)		           //Linha 07 
	//@ LI,56 PSAY TOTDESC PICTURE cPict1
	LI ++
	IF MONTH(dDataRef) = MONTH(SRA->RA_NASC)
		//	@ LI, 02 PSAY STR0038		//"F E L I Z   A N I V E R S A R I O  ! !"
		oPrint:Say(SetLin(nLin+28),SetCol(nCol:=55),STR0038,oFont09)		           //Linha 07
	ENDIF
	//oPrint:Say(SetLin(nLin+28),SetCol(nCol:=7),If( lLpt1 .And. !Empty(DESC_MSG3), fTAcento(DESC_MSG3), DESC_MSG3 ),oFont09)		           //Linha 07
	//@ LI,05 PSAY If( lLpt1 .And. !Empty(DESC_MSG3), fTAcento(DESC_MSG3), DESC_MSG3 )
	LI ++
Else
	oPrint:Say(SetLin(nLin+26),SetCol(nCol:=7),If( lLpt1 .And. !Empty(DESC_MSG1), fTAcento(DESC_MSG1), DESC_MSG1 ),oFont09)		           //Linha 07
	...
	oPrint:Say(SetLin(nLin+27),SetCol(nCol:=7),If( lLpt1 .And. !Empty(DESC_MSG2), fTAcento(DESC_MSG2), DESC_MSG2 ),oFont09)		           //Linha 07
	//@ LI,05 PSAY If( lLpt1 .And. !Empty(DESC_MSG2), fTAcento(DESC_MSG2), DESC_MSG2 )
	oPrint:Say(SetLin(nLin+26.5),SetCol(nCol:=127),TRANSFORM(TOTVENC,'@R 9,999,999,999.99'),oFont09)		           //Linha 07
	//@ LI,42 PSAY TOTVENC PICTURE cPict1
	oPrint:Say(SetLin(nLin+26.5),SetCol(nCol:=144),TRANSFORM(TOTDESC,'@R 9,999,999,999.99'),oFont09)		           //Linha 07
//@ LI,56 PSAY TOTDESC PICTURE cPict1
LI ++
oPrint:Say(SetLin(nLin+28),SetCol(nCol:=7),If( lLpt1 .And. !Empty(DESC_MSG3), fTAcento(DESC_MSG3), DESC_MSG3 ),oFont09)		           //Linha 07 
//@ LI,05 PSAY If( lLpt1 .And. !Empty(DESC_MSG3), fTAcento(DESC_MSG3), DESC_MSG3 )
...
IF MONTH(dDataRef) = MONTH(SRA->RA_NASC)
//	@ LI, 02 PSAY STR0038		//"F E L I Z   A N I V E R S A R I O  ! !"
   oPrint:Say(SetLin(nLin+29),SetCol(nCol:=55),STR0038,oFont09)		           //Linha 07 
ENDIF
Endif
oPrint:Say(SetLin(nLin+28.5),SetCol(nCol:=144),TRANSFORM(TOTVENC - TOTDESC,'@R 9,999,999,999.99'),oFont09)		           //Linha 07 
//@ LI,56 PSAY TOTVENC - TOTDESC PICTURE cPict1
LI +=2
...
EndIf
//@ LI,05 PSAY &cCompac+Transform(nValSal,cPict2)
oPrint:Say(SetLin(nLin+31.5),SetCol(nCol:=7),TRANSFORM(nValSal,'@R 9,999,999.99'),oFont09)		           //Linha 07 

If Esc = 1  // Bases de Adiantamento
	If cBaseAux = "S" .And. nBaseIr # 0
        oPrint:Say(SetLin(nLin+31.5),SetCol(nCol:=143),TRANSFORM(nBaseIr,'@R 9,999,999.99'),oFont09)		           //Linha 07 
//		@ LI,89 PSAY nBaseIr PICTURE cPict1
	Endif
ElseIf Esc = 2 .Or. Esc = 4  // Bases de Folha e 13o. 2o.Parc.
	If cBaseAux = "S"
        oPrint:Say(SetLin(nLin+31.5),SetCol(nCol:=38),TRANSFORM(nAteLim,'@R 9,999,999.99'),oFont09)		           //Linha 07 
//		@ LI,23 PSAY Transform(nAteLim,cPict1)
		If nBaseFgts # 0
            oPrint:Say(SetLin(nLin+31.5),SetCol(nCol:=66),TRANSFORM(nBaseFgts,'@R 9,999,999.99'),oFont09)		           //Linha 07 
//			@ LI,46 PSAY nBaseFgts PICTURE cPict1
		Endif
		If nFgts # 0
            oPrint:Say(SetLin(nLin+31.5),SetCol(nCol:=94),TRANSFORM(nFgts,'@R 9,999,999.99'),oFont09)		           //Linha 07 
//			@ LI,66 PSAY nFgts PICTURE cPict2
		Endif
		If nBaseIr # 	0
            oPrint:Say(SetLin(nLin+31.5),SetCol(nCol:=122),TRANSFORM(nBaseIr,'@R 9,999,999.99'),oFont09)		           //Linha 07 
//			@ LI,89 PSAY nBaseIr PICTURE cPict1
		Endif
        oPrint:Say(SetLin(nLin+31.5),SetCol(nCol:=143),TRANSFORM(nBaseIrfE,'@R 9,999,999.99'),oFont09)		           //Linha 07 
//		@ LI,103 PSAY Transform(nBaseIrfE,cPict1)
	Endif
ElseIf Esc = 3 // Bases de FGTS e FGTS Depositado da 1¦ Parcela
	If cBaseAux = "S"
...
IF SRA->RA_BCDEPSAL # SPACE(8)
	Desc_Bco := If( lLpt1, fTAcento(DescBco(Sra->Ra_BcDepSal,Sra->Ra_Filial)), DescBco(Sra->Ra_BcDepSal,Sra->Ra_Filial) )
    oPrint:Say(SetLin(nLin+34),SetCol(nCol:=120),STR0039,oFont09)		           //Linha 07
//	@ LI,01 PSAY STR0039	//"CRED:"
    oPrint:Say(SetLin(nLin+35),SetCol(nCol:=120),SRA->RA_BCDEPSAL,oFont09)		           //Linha 07
//	@ LI,06 PSAY SRA->RA_BCDEPSAL
    oPrint:Say(SetLin(nLin+35),SetCol(nCol:=128),"-",oFont09)		           //Linha 07
//	@ LI,14 PSAY "-"
    oPrint:Say(SetLin(nLin+35),SetCol(nCol:=129),DESC_BCO,oFont09)		           //Linha 07
//	@ LI,15 PSAY DESC_BCO
    oPrint:Say(SetLin(nLin+36),SetCol(nCol:=120),STR0040 + SRA->RA_CTDEPSAL,oFont09)		           //Linha 07
//	@ LI,50 PSAY STR0040 + SRA->RA_CTDEPSAL	//"CONTA:"
ENDIF
LI += 2
//@ LI,05 PSAY " "
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß*/
Static Function fContinGr(lLinhaRoda, nVia)    // Rodape do Recibo
...
Endif
LI ++
oPrint:Say(SetLin(nLin+27),SetCol(nCol:=7), 'CONTINUA' ,oFont12)		           //Linha 07 
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß*/
User Function OpenSrc( cMesArqRef, cAliasMov, aOrdBag, cArqMov, dDatRef, lContabil , lTerminal,cNomeSRI )
...
If !Empty( cCompetencia )
	...
	Else
		IF !lTerminal
			Aviso( STR0018, STR0019 + ": "+cArqMov  , { STR0021 } ) //##"Atencao"##"Nao existe arquivo de fechamento referente a data base solicitada"##"Ok"
		EndIf
		Return .f.
	EndIf
EndIf
...
IF !lTerminal
	nOpc := Aviso( STR0018, If(lContabil, STR0024, STR0020)+ ": "+cArqMov, { STR0022, STR0023 } ) //##"Atencao"##"Nao existe arquivo de fechamento referente a data base solicitada, deseja imprimir o relatorio do movimento atual?"##"Sim"##"Nao"
EndIf
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß/*/
User Function fExistArq(cArqRefe,cPathArq,nMesRefe,cArqMov)
...
If cPathArq == Nil
	...
	cPathArq := ""
	dbSelectArea("SX6")
	If dbSeek(Space(FWGETTAMFILIAL)+"MV_DGPESRC")
		cPathArq := SuperGetMv("MV_DGPESRC")
	EndIf
	If Empty(cPathArq)
		dbSelectArea("SX2")
		dbSeek("SRC")
		cPathArq := TRIM(Sx2->X2_PATH)
    EndIf
	dbSelectArea(cAlias)
EndIf
