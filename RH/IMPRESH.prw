...
±±∫Uso       ≥ AP6                                                                 ∫±±
±±»ÕÕÕÕÕÕÕÕÕÕœÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕº±±  
...
±±≥            ≥          ≥           ≥guraÁ„o do parametro que determina a impres-≥±± 
±±≥            ≥          ≥           ≥s„o da data de homologaÁ„o no campo 154 do  ≥±± 
±±≥            ≥          ≥           ≥TRCT.                                       ≥±± 
...
±±≥Mariana M   ≥23/10/2013|     THWPT4|Ajuste para que ao efetuar a geraÁ„o do     ≥±±
...
±±¿ƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ±± 
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
...
±±≥FunáÖo    ≥fHomolog≥ Autor ≥ Wagner Montenegro       ≥ Data ≥ 15.12.10 ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥DescriáÖo ≥ Impressao Formulario Homolonet                             ≥±±
...
Aadd(aCpoForm,{"73",2,"PrÍmios","008","",""})
Aadd(aCpoForm,{"74",2,"Viagens","010","",""})
Aadd(aCpoForm,{"75",2,"Sobreaviso","015","",""})
Aadd(aCpoForm,{"76",2,"Prontid„o","016","",""})
Aadd(aCpoForm,{"77",2,"Adicional por tempo de serviÁo","018","",""})
Aadd(aCpoForm,{"78",2,"Adicional por TransferÍncia de Localidade de Trabalho","019","",""})
Aadd(aCpoForm,{"79",2,"Sal·rio FamÌlia excedente ao Valor Legal","020","",""})
Aadd(aCpoForm,{"80",2,"Abono/GratificaÁ„o de FÈrias Excedente 20 dias de sal·rio","021","",""})
...
Aadd(aCpoForm,{"83",2,"Etapas marÌtimos","024","",""})
Aadd(aCpoForm,{"84",2,"LicenÁa PrÍmio indenizada","025","",""})
Aadd(aCpoForm,{"85",2,"Quebra de Caixa","026","",""})
Aadd(aCpoForm,{"86",2,"PLR","027","",""})
Aadd(aCpoForm,{"87",2,"IndenizaÁ„o a TÌtulo de Incentivo ‡ demiss„o","028","",""})
...
Aadd(aCpoForm,{"93",2,"GratificaÁ„o Semestral","034","",""})

Aadd(aCpoFor1,{"96",1,"IndenizaÁ„o art 9∫ Lei 7238/84","178","",""})
...
If n_X >= n_Y .and. n_X1 >= n_Y1 
    ...
    nProv-= n_X
    //-- Recompoe o valor de provento atravÈs do valor liquido do saldo de salario
    nProv+= n_XX
...
//  |52| Gratificacoes
// -------------------------------------------------------------------------------------------------------
oPrint:say (nL+nPT+10,nCl03a+15	,STR0046, oFont08)  //"52 GratificaÁ‚o"
...
// -------------------------------------------------------------------------------------------------------
//  |96| IndenizaÁ„o art. 9∫, Lei n∫ 7.238/1984 - A - |98| Multa art. 476-A & 5∫ da CLT"
// -------------------------------------------------------------------------------------------------------
For nY := 1 to Len(aCpoFor1)
	If Val(Strtran(StrTran(aCpoFor1[nY,5],".",""),",","."))>0 
		oPrint:line(nL,135,nL,nColMax )
...
For nX:=1 to nTamL
	...
	Elseif nX==2
		oPrint:say (nL+nPT,nCl01a+15		,STR0075, oFont08)  //"100 Pens„o AlimentÌcia"
	...
	Elseif nX==3
		oPrint:say (nL+nPT,nCl01a+15		,STR0078+" ", oFont08)  //"103 Aviso-PrÈvio Indenizado" 
		oPrint:say (nL+nPT+28,nCl01a+15		, StrZero(n_X2,2)+STR0151, oFont08)  //"103 Aviso-PrÈvio Indenizado"      
	    ...
	    nValLin:=2
	   	...
	   	IF Alltrim(GPER140Sum(2,2,"A08")) <> "0,00" 
	   		oPrint:say (nL+nPT,&("nCl0"+STR(nValLIn,1)+"a")+15	,STR0080, oFont08)  //"105 EmprÈstimo em ConsignaÁ„o"
		...
		IF Alltrim(GPER140Sum(2,2,"A04")) <> "0,00" 
	   	...
	   	IF Alltrim(GPER140Sum(2,2,"A05")) <> "0,00"
	   		oPrint:say (nL+nPT,&("nCl0"+STR(nValLIn,1)+"a")+15	,STR0083, oFont08)  //"108 Vale-AlimentaÁ„o"
			oPrint:say (nL+nPD,&("nCl0"+STR(nValLIn,1)+"b")+10	,GPER140Sum(2,2,"A05"), oFont10, , , , 1)
		    fDesconto(@nValLin)
	   	Endif
	   	IF Alltrim(GPER140Sum(2,2,"A07")) <> "0,00" 	 
			oPrint:say (nL+nPT,&("nCl0"+STR(nValLIn,1)+"a")+15  		,STR0084, oFont08)  //"109 Reembolso Vale-AlimentaÁ„o"
	   		oPrint:say (nL+nPD,&("nCl0"+STR(nValLIn,1)+"b")+10	,GPER140Sum(2,2,"A07"), oFont10, , , , 1)
	   		fDesconto(@nValLin)
		Endif
		IF Alltrim(GPER140Sum(2,2,"A11")) <> "0,00" 
			oPrint:say (nL+nPT,&("nCl0"+STR(nValLIn,1)+"a")+15	,STR0085, oFont08)  //"110 ContibuiÁ„o para o FAPI"
			oPrint:say (nL+nPD,&("nCl0"+STR(nValLIn,1)+"b")+10	,GPER140Sum(2,2,"A11"), oFont10, , , , 1)
			fDesconto(@nValLin)
		Endif
		IF Alltrim(GPER140Sum(2,2,"A13")) <> "0,00" 
			oPrint:say (nL+nPT,&("nCl0"+STR(nValLIn,1)+"a")+15	,STR0086, oFont08)  //"111 ContribuiÁ„o Sindical Laboral"
			oPrint:say (nL+nPD,&("nCl0"+STR(nValLIn,1)+"b")+10	,GPER140Sum(2,2,"A13"), oFont10, , , , 1)
			fDesconto(@nValLin)
		Endif
		oPrint:say (nL+nPT,&("nCl0"+STR(nValLIn,1)+"a")+15  	,STR0087, oFont08)  //"112.1 PrevidÍncia Social"
   		oPrint:say (nL+nPD,&("nCl0"+STR(nValLIn,1)+"b")+10	,GPER140Sum(2,1,"064/065"), oFont10, , , , 1)
   		fDesconto(@nValLin)
   		
  		oPrint:say (nL+nPT,&("nCl0"+STR(nValLIn,1)+"a")+15	,STR0088, oFont08)  //"112.2 PrevidÍncia Social 13∫ Salario"
   		oPrint:say (nL+nPD,&("nCl0"+STR(nValLIn,1)+"b")+10	,GPER140Sum(2,1,"070"), oFont10, , , , 1)
   		fDesconto(@nValLin)
	   		 
        IF Alltrim(GPER140Sum(2,2,"A10")) <> "0,00" 
        	oPrint:say (nL+nPT,&("nCl0"+STR(nValLIn,1)+"a")+15	,STR0089, oFont08)  //"113 ContribuiÁ„o PrevidÍncia "
		...
		IF Alltrim(GPER140Sum(2,1,"152")) <> "0,00" 
			oPrint:say (nL+nPT,&("nCl0"+STR(nValLIn,1)+"a")+15	,STR0093, oFont08)  //"114.3 IRRF sobre ParticipaÁ„o nos "
...
IF lTemVerba
	...
	oPrint:say (nL+nPT+20,nCl03a+15	,STR0099, oFont08n)  //"VALOR LÕQUIDO"
...
/*Categorias funcionarios considerados no TRCT atravÈs da portaria 1057/2012 	
01 - Empregado
...
//	oPrint:line(nL,035 ,nL,nColMax) 		//Linha Horizontal
	...
	nL := nL + nAddL + 05	
	oPrint:say (nL+nPT,160,STR0139	, oFont08) //31 CÛdigo Sindical                                      
...
If lHomolog
	nL -= 20
	oPrint:say (nL+nPT,140		,STR0141, oFont08)  //"Foi prestada, gratuitamente, assistÍncia na rescis„o do contrato de trabalho, nos termos do artigo n.∫ 477, ß 1∫, da ConsolidaÁao das Leis do Trabalho "                                                                                                                                                                                                                                                                                                                                                           
	nL += 35	
	oPrint:say (nL+nPT,140		,STR0142 , oFont08)  //"(CLT), sendo comprovado neste ato o efetivo pagamento das verbas rescisÛrias especificadas no corpo do TRCT, no valor lÌquido de R$ "
	nL += 35	
	oPrint:say (nL+nPT,140		,STR0157 + Transform(nProv - nDesc,"@E 999,999,999.99") + "," + STR0143, oFont08)  //"o qual, devidamente rubricado pelas partes, È parte integrante do presente Termo de HomologaÁao."    
	nL += 35
	
	nL += 35
	oPrint:say (nL+nPT,140		,STR0144, oFont08)  //"As partes assistidas no presente ato de rescis„o contratual foram identificadas como legÌtimas conforme previsto na InstruÁao Normativa/SRT"                                                                                                                                                                                                                                                                                                                                                                       
	nL += 35	
...
Else
	nL += 35
	oPrint:say (nL+nPT,140		,STR0125, oFont08)  //"Foi realizada a rescis„o do contrato de trabalho do trabalhador acima qualificado, nos termos do artigo n∫ 477 da ConsolidaÁ„o das Leis do Trabalho (CLT). "                                                                                                                                                                                                                                                                                                                                                       
	nL += 35
	oPrint:say (nL+nPT,140		,STR0126, oFont08)  //"A assistÍncia ‡ rescis„o prevista no ß 1∫ do art. n∫ 477 da CLT n„o È devida, tendo em vista a duraÁ„o do contrato de trabalho n„o ser superior a um ano "                                                                                                                                                                                                                                                                                                                                                         
	nL += 35
	oPrint:say (nL+nPT,140		,STR0127, oFont08)  //"de serviÁo e n„o existir previs„o de assistÍncia ‡ rescis„o contratual em Acordo ou ConvenÁ„o Coletiva de Trabalho da categoria a qual pertence o "                                                                                                                                                                                                                                                                                                                                                                
	nL += 35
	oPrint:say (nL+nPT,140		,STR0128, oFont08)  //"trabalhador."                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      
	
	nL += 70
	oPrint:say (nL+nPT,140		,STR0130 + Space(1)+ " ___/___/______ " + STR0131, oFont08) //"No dia __/__/____  foi realizado, nos termos do art. 23 da InstruÁ„o Normativa/SRT n∫ 15/2010, o efetivo pagamento das verbas rescisÛrias especificadas" 
	nL += 35
	oPrint:say (nL+nPT,140		,STR0132 + Space(2)+ Transform(nProv - nDesc,"@E 999,999,999.99") + STR0152, oFont08)  //"no corpo do TRCT, no valor lÌquido de R$ "##//", o qual, devidamente rubricado pelas partes, È parte integrante do presente Termo de QuitaÁ„o." 
	nL += 35
...
If !lHomolog
	oPrint:say (nL+(nAddL+nPT)+nPT,160,STR0135, oFont09)//-- "156 InformaÁıes ‡ CAIXA:"
Else
	oPrint:say (nL+((nAddL+nPT)*1.2)+25+nPT,160,STR0135, oFont09)//-- "156 InformaÁıes ‡ CAIXA:"
EndIf
...
oPrint:say (nL+nPD+10,180		,STR0116, oFont09n) //"Pode o trabalhador iniciar aÁ„o judicial quanto aos crÈditos resultantes das relaÁıes de trabalho atÈ o limite de dois anos apÛs a extinÁ„o do contrato de trabalho"
oPrint:say (nL+nPD+nPD+10,400	,STR0117, oFont09n) //"(Inc. XXIX, Art.7∫ da ConstituiÁ„o Federal/1988)."
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±⁄ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒø±±
±±≥FunáÖo    ≥fNewLine  ≥ Autor ≥ Kelly Soares          ≥ Data ≥ 20.01.11 ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥DescriáÖo ≥ Inicia nova linha na impressao das verbas rescisorias.     ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥Uso       ≥ RdMake                                                     ≥±±
±±¿ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±⁄ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒø±±
±±≥FunáÖo    ≥PenunSal  ≥ Autor ≥ Mauricio MR		    ≥ Data ≥ 16.02.11 ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥DescriáÖo ≥ Obtem o penultimo salario do funcionario antes da demissao.≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥Uso       ≥ IMPRESH                                                    ≥±±
±±¿ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±⁄ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒø±±
±±≥FunáÖo    ≥fVerQuebra≥ Autor ≥ Allyson M	            ≥ Data ≥ 25.03.13 ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥DescriáÖo ≥ Verifica se quebra linha e finaliza os box e ilinhas.      ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥Uso       ≥ RdMake                                                     ≥±±
±±¿ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±⁄ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒø±±
±±≥FunáÖo    ≥fCabec	≥ Autor ≥ Allyson M	            ≥ Data ≥ 06.05.13 ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥DescriáÖo ≥ Imprime o cabecalho da Rescisoa.      					  ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥Uso       ≥ RdMake                                                     ≥±±
±±¿ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
...
oPrint:say (nL+nAddL+05,155 ,"03 "+STR0005, oFont08)      	//"EndereÁo (Logradouro, n∫, andar, apartamento)"
...
oPrint:say (nL+nAddL+nPT+15,155 ,"12 "+STR0005, oFont08)  	//"EndereÁo (Logradouro, n∫, andar, apartamento)"
oPrint:say (nL+nAddL+nPT+15,1825,"13 "+STR0014, oFont08)	  	//"|04 Bairro : "
If SRA->(FieldPos("RA_NUMENDE")) # 0 .And. !Empty(SRA->RA_NUMENDE) 
	oPrint:say (nL+nAddL+nPD+15,175 ,Subs(SRA->RA_ENDEREC+', '+SRA->RA_NUMENDE+Space(40),1,40),oFont10) // "|03 Endereco + Numero de EndereÁo
Else
...
oPrint:say (nL+nPD,180	,If( ( ( SRA->RA_TPCONTR=="1" ) .or. Empty(SRA->RA_TPCONTR)),;
								...
								If(SRA->RA_CLAURES=="1", STR0011, STR0018)), oFont10)  //"2. Contrato de trabalho por prazo determinado com cl·usula assecuratÛria de direito recÌproco de rescis„o antecipada."##"3. Contrato de trabalho por prazo determinado sem cl·usula assecuratÛria de direito recÌproco de rescis„o antecipada."

nL:=nL+nTip-5
oPrint:say (nL+nPT,155,STR0019, oFont08) //"22 Causa do Afastamento" 
oPrint:say (nL+nPD,180,Left(cCausa,100), oFont10) 

oPrint:say (nL+nTip+nPT,155	,STR0020	, oFont08) //"23 RemuneraÁ„o MÍs Ant."
aAreaSRD:=GetArea()
...
oPrint:say (nL+nTip+nPT,1002+15,STR0022, oFont08) //"25 Data do Aviso PrÈvio"
...
oPrint:say (nL+nTip+nPT,1778+15,STR0155	, oFont08) //"27 CÛd. Afastamento"
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±⁄ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒø±±
±±≥FunáÖo    ≥fDesconto ≥ Autor ≥ Gustavo M		        ≥ Data ≥ 20/06/13 ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥DescriáÖo ≥ Quebra de linha para os descontos.						  ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥Uso       ≥ IMPRESH                                                    ≥±±
±±¿ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±⁄ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒø±±
±±≥FunáÖo    ≥fAjustaLin≥ Autor ≥ Claudinei Soares      ≥ Data ≥ 27/11/13 ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥DescriáÖo ≥Ajusta a impressao,caso existam linhas em branco disponiveis≥±±
...
if Val(Alltrim(GPER140Sum(2,3,"172/170/128/058/056") )) <> 0 	//"100 Pens„o AlimentÌcia"
	nRubDeduc ++
Endif
...
if Val(Alltrim(GPER140Sum(2,2,"A02") )) <> 0 					//"102 Adiantamento de 13∫ Sal·rio"
	nRubDeduc ++
Endif
if Val(Alltrim(GPER140Sum(2,1,"113") )) <> 0 					//"103 Aviso-PrÈvio Indenizado"
	nRubDeduc ++
Endif
if Val(Alltrim(GPER140Sum(2,1,"064/065"))) <> 0	 			//"112.1 PrevidÍncia Social"
	nRubDeduc ++
Endif
if Val(Alltrim(GPER140Sum(2,1,"070"))) <> 0 					//"112.2 PrevidÍncia Social 13∫ Salario"
	nRubDeduc ++
Endif
...
if Val(Alltrim(GPER140Sum(2,2,"A09") )) <> 0					//"104 Multa Art. 480/CLT"
	nRubDeduc ++
Endif
if Val(Alltrim(GPER140Sum(2,2,"A08"))) <> 0					//"105 EmprÈstimo em ConsignaÁ„o"
	nRubDeduc ++
Endif
...
if Val(Alltrim(GPER140Sum(2,2,"A05"))) <> 0					//"108 Vale-Alimentacao"
	nRubDeduc ++
Endif
if Val(Alltrim(GPER140Sum(2,2,"A07"))) <> 0					//"109 Reembolso Vale-AlimentaÁ„o" 	                                                          
	nRubDeduc ++
Endif
if Val(Alltrim(GPER140Sum(2,2,"A11"))) <> 0					//"110 ContibuiÁ„o para o FAPI" 
	nRubDeduc ++
Endif
if Val(Alltrim(GPER140Sum(2,2,"A13"))) <> 0					//"111 ContribuiÁ„o Sindical Laboral"	   		 
	nRubDeduc ++
Endif
if Val(Alltrim(GPER140Sum(2,2,"A10"))) <> 0					//"113 ContribuiÁ„o PrevidÍncia " 		
	nRubDeduc ++
Endif
if Val(Alltrim(GPER140Sum(2,1,"152"))) <> 0					//"114.3 IRRF sobre ParticipaÁ„o nos "
	nRubDeduc ++	
Endif
