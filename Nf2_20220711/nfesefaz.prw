			...
			Else
				...
				While !Eof() .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
					...
					If lContinua
						...
						Else
						    // Alteracao Multitek - Solucao Totvs nao Atende.
					...
					///EndIf
						...
						*/
						...
						//aadd(aPedCom,{})
					aadd(aPedCom,{"",""}) // Adaptacao Multitek 14-05-2013 - Manutencao no Vetor aPedCom 
						aadd(aPisAlqZ,{})
						aadd(aCofAlqZ,{})
						aadd(aCsosn,{})
	        // Adaptacao Multitek 05-04-2010 - Manutencao no Vetor aProd
			        aProd[LEN(aProd)][4] := execblock("DesCCProd",.f.,.f.,{cTipo,cSerie,cNota,cClieFor,cLoja,cAliasSD2,aProd[LEN(aProd)][4]})
             
					// Adaptacao Multitek 14-05-2013 - Manutencao no Vetor aPedCom
                    If  "NAO DETERMINADO" $ SC6->C6_X_PRCLI  
                        cC6_X_PRCLI := ''
                    Else
                        cC6_X_PRCLI := SC6->C6_X_PRCLI
                    Endif
 				    If !SD2->D2_TIPO $ "DB"
      			       If  alltrim(SA1->A1_X_GRPCO) $ "000019"  // grupo de contrato mrn (por Burin e Julio)
					       aPedCom[LEN(aPedCom)][1]:=Alltrim(substr(cC6_X_PRCLI,1,at("-",cC6_X_PRCLI)-1)) 
					       aPedCom[LEN(aPedCom)][2]:=Alltrim(substr(cC6_X_PRCLI,at("-",cC6_X_PRCLI)+1,6)) 
					   ElseIf alltrim(SA1->A1_X_GRPCO) $ "000021"
					       aPedCom[LEN(aPedCom)][1]:=cC6_X_PRCLI
					       aPedCom[LEN(aPedCom)][2]:='0'
					   Else                                  
					       aPedCom[LEN(aPedCom)][1]:=SC5->C5_X_COTCL 
					       if At("-",cC6_X_PRCLI) == 0
					          aPedCom[LEN(aPedCom)][2]:='0'
					       Else
					          aPedCom[LEN(aPedCom)][2]:=Alltrim(substr(cC6_X_PRCLI,at("-",cC6_X_PRCLI)+1,10)) 
					       Endif  
			    ...
			    If nValTFecp > 0
					...
					EndIf
			    EndIf
	        // Adaptacao Multitek 05-04-2010 
	                                                  
            cMensCli += execblock("MensNF",.f.,.f.,{cTipo,cSerie,cNota,cClieFor,cLoja})
			EndIf
		EndIf
	Next
Else
